-- WALMART_DB WITH STORED PROCEDURES , TRIGGERS, INDEXES AND QUERY OPTIMIZATION
...
-- TRIGGERS
--  1) Creating Trigger for USERS Table for data entry
...
VALUES (
    'USERS',                        -- Identifies the table where the action occurred
    'INSERT',                       -- Identifies the action type (INSERT)
    CONCAT(                         -- Concatenates the new user data for logging
        'user_id:', NEW.user_id,
...
-- 2) After Insert Trigger to Welcome New User
...
-- Sends a welcome message after a new user is inserted into the USERS table. (This example assumes there's a USER_MESSAGES table for storing messages.)
-- Step 1: Change delimiter temporarily
DELIMITER $$
...
BEGIN
    ...
    VALUES (NEW.user_id, 'Welcome to our platform!');
END$$

-- Step 3: Reset the delimiter to the default semicolon
DELIMITER ;
...
-- USER table
CREATE INDEX idx_email_id ON USERS(email_id);
...
DELIMITER //
CREATE PROCEDURE MonthlySalesReportByCategory(IN report_month INT, IN report_year INT)
BEGIN
...
-- Check Low Stock and Generate Reorder Alerts
DELIMITER //
...
-- Query optimization
-- 1. Query for Customers with the Most Purchases
-- Due to the index on user_id, another cool thing about this query is that it is cheap to gain the top 10 customers based on the number of orders they make.
EXPLAIN SELECT u.user_id, u.first_name, u.last_name, COUNT(o.order_id) AS purchase_count
FROM USERS u
...
-- 2. Query for High-Rating Products in Each Category
-- This query utilizes the indexes as well through joins of the various tables used through the category_id and product_id. Division by category and rating allow Walmart to find products most highly rated within each category.
EXPLAIN SELECT pc.category_name, p.product_name, AVG(r.rating) AS average_rating
FROM PRODUCT_CATEGORY pc
JOIN PRODUCTS p ON pc.category_id = p.category_id
JOIN REVIEWS r ON p.product_id = r.product_id
GROUP BY pc.category_name, p.product_name
HAVING average_rating >= 4
ORDER BY pc.category_name, average_rating DESC;
...
-- 3) Trigger to Log Deleted Orders
-- Stores deleted orders in an ORDERS_ARCHIVE table to keep a record of removed data.
...
FOR EACH ROW 
BEGIN
    INSERT INTO ORDERS_ARCHIVE (order_id, user_id, total_amount)
    VALUES (OLD.order_id, OLD.user_id, OLD.total_amount);
END$$
